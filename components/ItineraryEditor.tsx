...
type Props = { value: Day[]; onChange: (days: Day[]) => void; startDate?: string }

export default function ItineraryEditor({ value, onChange, startDate }: Props) {
  ...
  function formatEstimate(start?: string, end?: string){
    const a = parseTime(start), b = parseTime(end)
    if(a!=null && b!=null){ let d=b-a; if(d<0) d+=1440; if(d<=0) return '—'; const h=Math.floor(d/60), m=d%60; return h?`${h}h ${m?m+'m':''}`:`${m}m` }
    return '—'
  }
                                    ...
                                    {editing?.activityId === a.id ? (
                                      ...
                                      <div className="flex-1">
                                        ...
                                        <div className="mt-2 flex items-center gap-2">
                                          <input className="input w-28" placeholder="Start (09:00)" value={a.start || ''} onChange={e=>saveActivity(day.id, a.id, { start: e.target.value })} />
                                          <span>–</span>
                                          <input className="input w-28" placeholder="End (11:30)" value={a.end || ''} onChange={e=>saveActivity(day.id, a.id, { end: e.target.value })} />
                                          <span className="text-xs opacity-70 ml-2">Est: {formatEstimate(a.start, a.end)}</span>
                                        </div>
...
function InlineActivityEditor({ initial, onSave, onCancel }:{ initial: Activity; onSave:(patch:Partial<Activity>)=>void; onCancel:()=>void }){
  ...
  return (
    <form className="flex-1 flex flex-col gap-2" onSubmit={(e) => { e.preventDefault(); onSave({ text }) }}>
